...
struct ShengDetailView: View {
    // MARK: - PROPERTIES
    @Environment(\.modelContext) private var modelContext
    @Query var sections: [ShengSection]
    ...
    init(order: Int?, toolbarTitle: String, toolbarIcon: String) {
        ...
        if let sectionOrder = order {
            _sections = Query(filter: #Predicate { $0.order == sectionOrder }, sort: \ShengSection.order)
        } else {
            _sections = Query(filter: #Predicate { $0.order == -1 }, sort: \ShengSection.order)
        }
    }
...
// MARK: - PREVIEW
struct ShengDetailView_Previews: PreviewProvider {
    static var previews: some View {
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        
        do {
            let container = try ModelContainer(for: ShengSection.self, ShengSubsection.self, ShengText.self, configurations: config)
