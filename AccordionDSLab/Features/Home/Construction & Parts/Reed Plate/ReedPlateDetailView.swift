...
struct ReedPlateDetailView: View {
    // MARK: - PROPERTIES
    @Environment(\.modelContext) private var modelContext
    @Query var sections: [ReedPlateSection]
    ...
    init(order: Int?, toolbarTitle: String, toolbarIcon: String) {
        ...
        if let sectionOrder = order {
            _sections = Query(filter: #Predicate { $0.order == sectionOrder }, sort: \ReedPlateSection.order)
        } else {
            _sections = Query(filter: #Predicate { $0.order == -1 }, sort: \ReedPlateSection.order)
        }
    }
...
// MARK: - PREVIEW
struct FreeReedDetailView_Previews: PreviewProvider {
